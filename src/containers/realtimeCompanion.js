...
class RealtimeCompanion extends Component {
  constructor(props) {
    super(props);
    this.state = { 
      performances: [], 
    ...
    this.handleRealtimeInstant = this.handleRealtimeInstant.bind(this);
    this.eventSource = new EventSource(SSE_PROVIDER); // event source for real-time server-sent events (SSE)
  }
...
//      this.assignClickHandlersToNotes();
  ...
  handleRealtimeInstant(instant) { 
    ...
    let noteToFlipTo = "";
    let currentMeasure = "";
    // TODO package this noteIds.map into a function, and refactor identical code in tickTimedResource
    noteIds.map((n) => { 
    ...
    // highlight the current note if on current page
      ...
      if(currentNoteElement) { 
        ...
        if(this.state.showVelocities) { 
          let mappedVel = this.mapVelocity(instantJson["https://terms.trompamusic.eu/maps#velocity"])
        ...
        // (only if we are showing alignment confidence)
            currentMeasure.classList.add("errorDetected");
          // and clear the animation a second later (so that we can punish the next pianist that gets this meausure wrong!)
            setTimeout( (element) => { 
              element.classList.remove("errorDetected");

            }, 1000, currentMeasure); 
      } else if(currentNoteId !== "insert_state") { 
