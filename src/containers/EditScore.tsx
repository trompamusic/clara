...
import { PerformanceShapeShapeType, SignalShapeShapeType } from '../.ldo/performance.shapeTypes';
...
const PerformanceRow: FunctionComponent<{
    ...
    const performanceData = useSubject(PerformanceShapeShapeType, performance.uri);
    ...
    return (
            <tr>
                <td><a href={performance.uri} target="_blank" rel="noopener noreferrer">{performance.id}</a></td>
...
const EditScore: FunctionComponent = () => {
    ...
    const [deletingPerformance, setDeletingPerformance] = useState<string | null>(null);
    ...
    const performances = useSubject(ScoreShapeShapeType, performancesUri || undefined);

    // Fetch performances when the performances container is available
    React.useEffect(() => {
        if (performancesResource && 'children' in performancesResource && performancesResource.isPresent()) {
            const fetchPerformances = async () => {
                setIsLoadingPerformances(true);
                try {
                    const children = await performancesResource.children();
                    console.log('Found performance children:', children.map((c: any) => c.uri));
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (score?.related?.['@id']) {
                const performancesUri = score.related['@id'];
                console.log('Deleting all performances from container:', performancesUri);
                try {
                ...
                try {
                    ...
                    if (performancesResource && 'delete' in performancesResource) {
                        ...
                        if (!result.isError) {
                            ...
                            console.error('Failed to delete performances container:', result.message);
                        }
                    } else {
    ...
    const deleteSinglePerformance = async (performanceUri: string, signalUri?: string) => {
        ...
        try {
            ...
            if (performanceUri.includes('/performances/')) {
                const timelineUri = performanceUri.replace('/performances/', '/timelines/');
    ...
    const handleDeletePerformance = async (performanceUri: string, signalUri?: string) => {
        ...
        if (success) {
            ...
            setPerformancesList(prev => prev.filter(p => p.uri !== performanceUri));
        }
    ...
    if (scoreResource && 'isReading' in scoreResource && scoreResource.isReading()) {
        return (
            <div className="container mt-4">
                            ...
                            <dl className="row">
                                ...
                                <dd className="col-sm-8">
                                    {displayData?.originalUrl !== 'No original URL' ? (
                                        <a href={displayData?.originalUrl} target="_blank" rel="noopener noreferrer">
                            ...
                            <dl className="row">
                                ...
                                <dd className="col-sm-8">
                                    {displayData?.hasSegments ? (
                                        <a href={displayData?.segmentsUrl} target="_blank" rel="noopener noreferrer">
                                ...
                                <dd className="col-sm-8">
                                    {displayData?.hasPerformances ? (
                                        <a href={displayData?.performancesUrl} target="_blank" rel="noopener noreferrer">
            ...
            <div className="mt-4">
                ...
                <div className="alert alert-danger d-flex justify-content-between align-items-center">
                    {!showDeleteConfirm ? (
                        ...
                        <div>
                            <p className="mb-0"><strong>Are you sure you want to delete this score?</strong></p>
                        </div>
                    )}
                    {!showDeleteConfirm ? (
                        <Button 
